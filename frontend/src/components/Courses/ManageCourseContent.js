...
const ManageCourseContent = () => {
    ...
    const [lectureModalState, setLectureModalState] = useState({ isOpen: false, moduleId: null, lectureToEdit: null });
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 pb-12">
            ...
            <div className="space-y-6">
                {modules.length > 0 ? modules.map((module) => (
                    <div key={module._id} className={`card border-l-4 transition-colors ${module.isPublished ? 'border-l-blue-500' : 'border-l-gray-300'}`}>
                        ...
                        <div className="space-y-2 ml-0 sm:ml-9">
                            {module.lectures && module.lectures.sort((a,b) => a.order - b.order).map((lecture) => {
                                const Icon = getIconForType(lecture.resources.length > 0 ? lecture.resources[0].type : 'document');
                                return (
                    ...
                    <div className="text-center py-16 card border-2 border-dashed">
                        ...
                        <button onClick={handleAddModule} className="btn btn-primary px-6"><PlusIcon className="h-5 w-5 mr-2" /> Create First Module</button>
                    </div>
                )}
            </div>
            ...
            {lectureModalState.isOpen && (<LectureForm moduleId={lectureModalState.moduleId} lecture={lectureModalState.lectureToEdit} onClose={closeLectureModal} onSave={() => { closeLectureModal(); fetchData(); }} />)}
        </div>
    );
};
